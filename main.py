...
class Profile(object):
    ...
    def add_eMOLT_header(self, filename, data, sensor):
        ...
        os.system('gzip -c -9 '+new_filename +' > ' +self.path + 'merged/zip/' + 'zt_{SN}_{date}_{vessel}.gz'.format(SN=MAC_FILTER[0][-5:], date=date_file, vessel=vessel_name))
        #os.system('gzip -c '+new_filename +' > ' self.path + 'merged/zip/' + 'zt_{SN}_{date}_{vessel}+.gz'.format(SN=MAC_FILTER[0][-5:], date=date_file, vessel=vessel_name))
    ...
    def eMOLT_data_trans(self, lMOLT): 
        ...
        for filename, data in lMOLT:
            ...
            daily_ave=''
            if 'f' in self.gear or 'F' in self.gear:
                try:    
                        print ("it is Fixed")
    ...
    def wifi(self):
      ...
      if 3>2:
            files=[]
            files.extend(sorted(glob.glob(self.path+'merged/zip/*.gz')))
    ...
    def scp(self):
      ...
      if 3>2:
            ...
            try:
                ...
                print ('try to connect scp')
                time.sleep(3)# cannot send data too frequently 
                c.connect(hostname='76.24.249.119',port='1111',username='pi-emolt',pkey=k)
                print ('connected')
                sftp=c.open_sftp()
                time.sleep(2) # cannot send data or connect too frequently, need time sleep between two steps
                for u in dif_data:
                    import time
                    #print (u)
                    sftp.put(u, '/home/pi-emolt/all_raw/compressed/'+u.split('/')[-1])
                    time.sleep(1)                     # close file and FTP
