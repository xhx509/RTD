...
class GPS(object):
    ...
    def store_all_csv(self):
        ...
        if len(self.df_total != 0):
            ...
            gps['LATITUDE'] = round(self.df_total.apply(lambda x: round(int(x['LATITUDE'] / 100) + (x['LATITUDE'] / 100 % 1) * 100 / 60, 6) if x['latd'] == 'N' else -round(int(x['LATITUDE'] / 100) + (x['LATITUDE'] / 100 % 1) * 100 / 60, 6), axis=1),6)
            ...
            gps['LONGITUDE'] = round(self.df_total.apply(lambda x: round(int(x['LONGITUDE'] / 100) + (x['LONGITUDE'] / 100 % 1) * 100 / 60, 6) if x['lond'] == 'E' else -round(int(x['LONGITUDE'] / 100) + (x['LONGITUDE'] / 100 % 1) * 100 / 60, 6), axis=1),6)
