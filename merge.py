...
class Merge(Load):
    ...
    def merge_mobile(self, GPS, sensor):  # GPS and sensors are DataFrames
        ...
        time_s = sensor['DATETIME'].iloc[0] - timedelta(seconds=1)  # time starts
        #time_e = sensor['DATETIME'].iloc[-1] + timedelta(seconds=5)  # time ends
        time_e = sensor['DATETIME'].iloc[-1]- timedelta(seconds=1)
    ...
    def merge_fixed(self, GPS, sensor):
        fixed_lat=round(GPS['LATITUDE'].iloc[-1],6)
        fixed_lon=round(GPS['LONGITUDE'].iloc[-1],6)
        '''
